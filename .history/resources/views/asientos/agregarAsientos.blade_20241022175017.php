    ...
    <script type="text/javascript">
        ...
        function verificarBalance() {
            ...
            if (totalDebe === totalHaber) {
                alert(`El balance es correcto. Total Debe: ${totalDebe}, Total Haber: ${totalHaber}`);
            } else {
                alert(`El balance NO es correcto. Total Debe: ${totalDebe}, Total Haber: ${totalHaber}`);
            }
        }
        ...
        function existeCuenta(nombreCuenta, tipo) {
            const filas = tablaCuentas.getElementsByTagName('tr');
            for (let fila of filas) {
                const celdaCuenta = fila.getElementsByTagName('td')[0]; // Primera celda es la cuenta
                const celdaDebe = fila.getElementsByTagName('td')[1]; // Segunda celda es "Debe"
                const celdaHaber = fila.getElementsByTagName('td')[2]; // Tercera celda es "Haber"

                if (celdaCuenta && celdaCuenta.innerText === nombreCuenta) {
                    // Verificar si el tipo de cuenta coincide
                    if (tipo === "Debe" && celdaDebe.innerText !== "0") {
                        return {
                            existe: true,
                            tipoCoincide: true,
                            fila
                        }; // Existe y es "Debe"
                    } else if (tipo === "Haber" && celdaHaber.innerText !== "0") {
                        return {
                            existe: true,
                            tipoCoincide: true,
                            fila
